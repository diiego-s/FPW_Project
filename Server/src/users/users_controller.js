...
const addUser = (req, res) => {
    const {username, password, name, surname, email, city, favcity, age} = req.body;
    pool.query(queries.getUserByUsername, [username], (error, result) => {
        if(error) throw error;
        if(result.rows.length > 0) {
            res.status(400).json({message: 'User already exists'});
        } else {
            pool.query(queries.addUser, [username, password, name, surname, email, city, favcity, age], (error, result) => {
                if(error) throw error;
